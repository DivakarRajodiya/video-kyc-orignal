...
class ConfigController extends AppBaseController
{
    ...
    {
        $fileName = !is_null($request->query('file')) ? $request->query('file') : 'config.json';

        return view('configs.index', compact('fileName'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $input = $request->all();
            $jsonString = file_get_contents(public_path('json/config.json'));
            file_put_contents(public_path('json/') . $input['fileName'] . '.json', $jsonString);
    ...
    public function update(Request $request)
    {
        try {
            ...
            $input = $request->all();
            $jsonString = file_get_contents(public_path('json/'.$input['fileName']));
            ...
            file_put_contents(public_path('json/'.$input['fileName']), $newJsonString);
    ...
    public function destroy(Request $request)
    {
        try {
            ...
            $input = $request->all();
            @unlink(public_path('json/') . $input['fileName'] . '.json');
